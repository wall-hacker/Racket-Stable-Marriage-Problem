...
(define (get-women wpref)
  (foldr (lambda (x acc) (cons (first x) acc)) '() wpref))
...
(define (get-pref-list pref person)
  (foldl (λ (x acm) (if (equal? (car x) person)
                        (cdr x)
                        acm))
         null pref))
...
(define (preferable? pref-list x y)
    (if (and (member x pref-list) (member y pref-list))
...
(define (get-partner engagements person)
  (define answer (find-first (lambda (x) (equal? (car x) person)) engagements))
  (if answer
      (cdr answer)
      #f))
...
(define (better-match-exists? p1 p2 p1-list pref2 engagements)
  (if (or (null? p1-list) (equal? (car p1-list) p2))
      #f
      (if (better-match-exists? p1 p2 (cdr p1-list) pref2 engagements)
          #t
...
(define (stable-match? engagements mpref wpref)
  (andmap (lambda (x) (nor (better-match-exists? (cdr x) (car x) (get-pref-list mpref (cdr x)) wpref engagements)
                          (better-match-exists? (car x) (cdr x) (get-pref-list wpref (car x)) mpref engagements))) engagements))
