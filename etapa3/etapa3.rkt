...
(define (flip-engagements lst)
  (map (lambda (pair) (cons (cdr pair) (car pair))) lst))

(define (get-unstable-couples engagements mpref wpref)
  (let loop ((curr-engagements engagements)
             (unstable-couples '()))
    (cond ((null? curr-engagements)
           unstable-couples)
          (else
           (let* ((couple (car curr-engagements))
                  (rest-engagements (cdr curr-engagements))
                  (woman (car couple))
                  (man (cdr couple))
                  (man-pref (get-pref-list mpref man))
                  (woman-pref (get-pref-list wpref woman)))
             (if (or (better-match-exists? man woman man-pref wpref engagements)
                     (better-match-exists? woman man woman-pref mpref (flip-engagements engagements)))
                 (loop rest-engagements (cons couple unstable-couples))
                 (loop rest-engagements unstable-couples)))))))
...
(define (engage free-men engagements mpref wpref)
  (let loop1 ((free-men free-men) (eng engagements))
    (cond ((null? free-men) eng)
          (else (let loop2 ((free-man-pref (get-pref-list mpref (car free-men))))
                  (cond ((equal? #f (get-partner eng (car free-man-pref))) (loop1 (cdr free-men) (cons (cons (car free-man-pref) (car free-men)) eng)))
                        (else (if (preferable? (get-pref-list wpref (car free-man-pref)) (car free-men) (get-partner eng (car free-man-pref))) (loop1 (cons (get-partner eng (car free-man-pref)) (cdr free-men)) (update-engagements eng (car free-man-pref) (car free-men)))
                                  (loop2 (cdr free-man-pref))))))))))
                        
    
             
;(define (engage free-men engagements mpref wpref)
;(get-pref-list mpref (car free-men)))
