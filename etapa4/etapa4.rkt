...
(define (ff-pers-in-room person eng)
  (define answer (find-first (lambda (x) (equal? (car x) person)) eng))
  (if answer
      (car answer)
      #f))
  ...
  (let loop1 ((p person) (eng engagements))
    (if (null? p) eng
        (let loop2 ((p-list (get-pref-list pref1 p)))
          (if (null? p-list) (loop1 '() (cons (cons #f p) eng))
              (let ([pp (ff-pers-in-room (car p-list) eng)])
                (if (equal? pp #f) (loop2 (cdr p-list))
                    (let ([ppp (get-partner eng pp)])
                      (if (equal? ppp #f) (loop1 '() (update-engagements eng pp p))
                          (if (preferable? (get-pref-list pref2 pp) p ppp) (loop1 ppp (update-engagements eng pp p))
                              (loop2 (cdr p-list))))))))))))
...
(define (is-first-in-lists? person lists)
  (ormap (lambda (lst) (equal? (first lst) person)) lists))

(define (path-to-stability engagements mpref wpref queue)
  (let loop ((eng engagements) (q queue))
    (if (null? q) 
        eng
        (let* ([p (car q)] [is-man (is-first-in-lists? p mpref)])
          (if (equal? #t is-man) (loop (match p eng mpref wpref (cdr q)) (cdr q))
              (loop (flip-engagements (match p (flip-engagements eng) wpref mpref (cdr q))) (cdr q)))))))
...
(define (update-stable-match engagements mpref wpref)
  (let* ([unstable (get-unstable-couples engagements mpref wpref)]
         [room-engagements (list-subtract engagements unstable)]
...
(define (build-stable-matches-stream pref-stream)
  ...
  (stream-cons (gale-shapley (car (stream-first pref-stream)) (cdr (stream-first pref-stream)))
               (stream-zip-with (lambda (prev pref) (let* ([mpref (car pref)] [wpref (cdr pref)]) (update-stable-match prev mpref wpref))) (build-stable-matches-stream pref-stream) (stream-rest pref-stream)))))
