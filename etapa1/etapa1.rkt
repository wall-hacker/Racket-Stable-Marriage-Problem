...
(define (tail-recursion wpref aux)
  (cond
    ((empty? wpref) aux)
    (else (tail-recursion (cdr wpref) (append aux (list (car (car wpref))))))))

(define (get-women wpref)
  (tail-recursion wpref '()))
...
;  (if (equal? (car pref-list) x)
;      #t
;      (if (equal? (car pref-list) y)
;          #f
